      ...
      <main style={{ background: '#000', minHeight: '100vh', fontFamily: "'JetBrains Mono', monospace" }}>
        ...
        <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 40px', borderBottom: '0.5px solid #1a1a1a', background: 'rgba(0,0,0,0.96)' }}>
          ...
          <div style={{ display: 'flex', gap: '36px' }}>
            {['Home', 'Free Tools', 'Contact'].map((item) => (
              <span
                key={item}
...
<div key={name} style={{ background: '#000', padding: '30px', display: 'flex', flexDirection: 'column', gap: '10px', minHeight: '200px', opacity: 0.3 }}>
  <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', color: '#444', textTransform: 'uppercase' }}>// Coming Soon · Powered by AIS</div>
  <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '14px', fontWeight: 700, color: '#fff', letterSpacing: '0.04em' }}>{name}</div>
  <div style={{ fontSize: '11px', color: '#777', lineHeight: 1.7, flex: 1 }}>Real-time AI analysis of surveillance footage with natural language search. Built on AIS — reduce 60 days of manual footage review to under a week. Query any camera feed like a conversation.</div>
</div>            ))}
