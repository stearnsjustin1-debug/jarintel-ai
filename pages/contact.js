...
export default function Contact() {
  ...
  const navItems = ['Home', 'Free Tools', 'Contact']
  ...
  return (
    <>
      ...
      <main style={{ background: '#000', minHeight: '100vh', fontFamily: "'JetBrains Mono', monospace" }}>
        ...
        <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 40px', borderBottom: '0.5px solid #1a1a1a', background: 'rgba(0,0,0,0.96)' }}>
          ...
          <div style={{ display: 'flex', gap: '36px' }}>
            {navItems.map(item => (
              <span
                key={item}
                onClick={() => router.push(item === 'Home' ? '/' : item === 'Free Tools' ? '/free-tools' : '/contact')}
                style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: item === 'Contact' ? '#fff' : '#888', cursor: 'pointer' }}
                onMouseEnter={e => { if (item !== 'Contact') e.target.style.color = '#fff' }}
                onMouseLeave={e => { if (item !== 'Contact') e.target.style.color = '#888' }}
              >
                {item}
              </span>
            ))}
        ...
        {/* CONTENT */}
        <div style={{ padding: '100px 40px 100px', maxWidth: '640px', margin: '0 auto' }}>
          ...
          {submitted ? (
            <div style={{ background: '#080808', border: '0.5px solid #1a1a1a', padding: '40px' }}>
              ...
              <div style={{ fontSize: '11px', color: '#bbb', lineHeight: 1.9, marginBottom: '28px' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: '#1a1a1a' }}>
                <div style={{ background: '#000', padding: '0 0 0 0' }}>
                  ...
                  <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="John Smith" required />
                </div>
                <div style={{ background: '#000', paddingLeft: '24px' }}>
                  <label style={labelStyle}>Organization</label>
                  <input style={inputStyle} value={organization} onChange={e => setOrganization(e.target.value)} placeholder="Metro Police Department" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: '#1a1a1a' }}>
                ...
                <div style={{ background: '#000', paddingLeft: '24px' }}>
                  ...
                  <input style={inputStyle} value={location} onChange={e => setLocation(e.target.value)} placeholder="Chicago, IL" />
                </div>
              </div>
